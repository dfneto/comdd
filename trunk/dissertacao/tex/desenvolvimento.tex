...
\begin{figure}
	\centering
		\includegraphics[width=15cm]{imagens/arquitetura.png}
...
\section{Método}
\label{metodo}
A aceitação de estudos empíricos na engenharia de software está crescendo, pois a pesquisa analítica não é o suficiente para investigar a real complexidade entre humanos e sua interação com a tecnologia \cite{Runeson2009}. Deste forma, este trabalho realiza como método de pesquisa a elaboração e execução de estudo de caso.

Segundo Runenson \cite{Runeson2009}, um estudo de caso é um método empírico com o intuito de investigar um fenômeno em um contexto. Cesar \cite{Cesar} define um estudo de caso como sendo uma abordagem qualitativa para coleta de dados a cerca de um fenômeno a ser investigado. Cesar também afirma que um estudo de caso deve ter três fases distintas: o desenvolvimento de um protocolo que norteie o estudo; coleta e análise dos dados e a interpretação desses dados. Assim, este trabalho se baseou parcialmente nos trabalhos de Runeson e Cesar.

Foram definidos desenvolvidos três estudos de caso: EC-ND (Estudo de Caso dos Não Desenvolvedores), EC-PG (Estudo de Caso dos Pós Graduandos) e EC-IN (Estudo de Caso da Indústria)\footnote{Nas respectivas seções de cada estudo de caso será apresentado o perfil dos participantes, sendo ND, PG e IN generalizações com o intuito de facilitar a referência aos estudos de caso.}. 

O EC-ND tinha o intuito de avaliar o CoMDD por não desenvolvedores e com um grau de escolaridade mediano\footnote{Não faz-se aqui uma análise da escolaridade nacional, tampouco deseja-se diminuir ou inferiorizar o nível de escolaridade dos participantes. Classificou-se como mediano pois os participantes já haviam concluído a maior parte do ensino médio, entretanto ainda não haviam feito graduação.}. Desta forma seria possível avaliar o nível de aceitação por não desenvolvedores.

O EC-PG tinha a intenção de avaliar o CoMDD por alunos de pós graduação em ciências da computação, e formação também em ciências da computação, e que já tivessem uma experiência mínima de um ano na indústria\footnote{Neste trabalho, o termo indústria se refere à indústria de desenvolvimento de software.}.  Desta forma, os resultados do EC-PG teriam uma visão intermediária entre academia e indústria.
...
Assim, este trabalho realizou três estudos de caso com o intuito de evidenciar a \tetbf{hipótese de que é possível usar uma wiki para desenvolver um sistema colaborativamente usando MDD}. 
...
\subsubsection{Conclusões e observações do EC-ND}
...
Como aprendizado do EC-ND é necessário planejar melhor em dois aspéctos:
\begin{enumerate}
...
O trabalho do CoMDD e do Galaxy Wiki estão muito próximos, uma vez que ambos trabalham se atentam para o uso de wikis que forneçam o suporte necessário e independente para que desenvolvedores possam programar. Contudo, a principal diferença entre as abordagens está na natureza do CoMDD. Usar modelos ao invés de linguagens de programação tem com o intuito, além de aumentar a produtividade do desenvolvimento\footnote{Neste caso está se referindo que uma abordagem na wiki usando modelos é mais produtiva do que uma mesma abordagem na wiki mas usando linguagens de programação. Ver vantagens do MDD na Seção \ref{}}, permitir que não desenvolvedores possam colaborar. Embora os próprios autores do Galaxy Wiki citem essa como uma possível vantagem do Galaxy Wiki, nota-se ao longo do artigo que este não é um dos focos principais. 

No caso do CoMDD ele foi projetado e pensado como um dos seus objetivos o de ser acessível por não desenvolvedores e este é um dos motivos de uso de MDD no lugar de Java, por exemplo. 

O principal diferencial é que enquanto o Galaxy Wiki permite o desenvolvimento de código em java e a integração com o Eclipse, o CoMDD trabalha em um nível de abstração maior e não permite a integração com IDEs.

Este trabalho apresentou um ambiente baseado em uma wiki, na qual programadores podem colaborativamente escrever códigos-fonte, construir projetos, executar programas e debugar defeitos.
