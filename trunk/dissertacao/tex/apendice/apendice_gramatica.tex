...
\chapter{Gramática do CoMDD}
\label{apendice_gramatica}
...
@init{
	...
	: stat+ -> classe(cab={$robo::cabList}, 
		pacote={$robo::pacList}, sen={$robo::senList}, 
			main={$robo::mainList})
	;
...
cabecalho:
	'adicionar' itensCabecalho=(
		'defines' -> define() 
		| 'includes' -> include() )
//	'adicionar defines' ({$nomeDaPlataforma::name == null}? 
				-> definePionner())
//			    -> defineSrv() )
...
//*****************************************/
main:
	('int main()' '{' -> mainTemplate()) 
			{$robo::mainList.add($main.st);}  loop '}'
	;
...
andar	:	('andar();' -> 
		andar()) {$robo::mainList.add($andar.st);}
	;
