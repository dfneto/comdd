...
Portanto, o modelo no MDD é um artefato primordial para o desenvolvimento e dessa forma se contrapõe ao desenvolvimento convencional de software, no qual os modelos são utilizados para representar o problema e servem \emph{apenas} para auxiliar o desenvolvimento de código pelo programador, ou seja, os modelos são usados \emph{somente} como apoio para o desenvolvimento do código. No MDD, a idéia é que esses modelos possam gerar o código (e outros artefatos) e não fiquem limitados apenas como um auxílio ao desenvolvedor, conforme ilustram as Figuras \ref{fig:desTrad} e \ref{fig:desMDD} em que o desenvolvedor usa modelos como auxílio, na Figura \ref{fig:desTrad}, em detrimento a Figura \ref{fig:desMDD}, em que os modelos são os responsáveis por gerar o código.
...
\item Manutenção: alterações no código respectivas à manutenção podem requerer o mesmo esforço produzido durante o desenvolvimento;
...
O termo DSML (\textit{Domain-Specific Modeling Language}) pode ser considerado como um sinônimo de DSL e para \cite{Heering2008} DSML é uma DSL no contexto de MDD; com uma sintaxe gráfica, geralmente.
...
As DSMLs permitem os desenvolvedores usarem conceitos específicos do domínio, por isso fornecem uma linguagem de modelagem ainda mais intuitiva e integram  ainda mais o \textit{know-how} dos desenvolvedores de software e as otimizações específicas de aplicações dentro dos geradores de código. Por exemplo: um desenho, de um especialista de domínio, no Visio\footnote{http://office.microsoft.com/en-us/visio/default.aspx} é um modelo; com sintaxe e semântica clara, que permite a geração de código baseadas na especificação do modelos \cite{Liggesmeyer2009}.
...
Fazendo um paralelo entre as duas abordagens, na abordagem Eclipse, a linguagem de modelagem é o EMF, enquanto que na MDA é a UML. A linguagem de meta-metamodelagem na abordagem Eclipse é o \emph{Ecore} e na MDA é o MOF. A Eclipse também usa a especificação XMI, da OMG, para fazer o intercâmbio de modelos.
